...
void solve_poisson(std::vector<std::vector<double>> &V, int M, int N, double h, double k, double tol, int &iterations, const std::string& schedule_type) {
    double delta = 1.0;
    iterations = 0;
...
int main() {
    ...
    std::cin >> num_threads;
    std::cout << "Ingrese schedule (static/dynamic): ";
    std::cin >> schedule_type;

    if (M <= 0 || N <= 0 || num_threads < 1 || (schedule_type != "static" && schedule_type != "dynamic")) {
        std::cerr << "Parámetros inválidos.\n";
        return 1;
    }

    omp_set_num_threads(num_threads);
    std::cout << "Resolviendo con " << num_threads << " hilos y schedule " << schedule_type << "...\n";
    ...
    solve_poisson(V, M, N, h, k, 1e-6, iterations, schedule_type);
    ...
    std::string outputfile = "poisson_schedule3.png";
