...
//   Ahora con paralelización OpenMP usando collapse(2) y cálculo de delta.
// ------------------------------------------------
...
std::string export_to_csv(const std::vector<std::vector<double>> &V,
                          ...
                          int case_num)
{
    // Nombre solicitado: poisson_collapse_caso_<case_num>_<MxN>.csv
    std::string filename = "poisson_collapse_caso_" +
...
int main() {
    ...
    // Los archivos se guardan como poisson_collapse_caso_<case>_<MxN>.csv/.png
    std::string csv_file = export_to_csv(V, M, N, h, k, case_selector);
    std::string plot_file = "poisson_collapse_caso_" +
