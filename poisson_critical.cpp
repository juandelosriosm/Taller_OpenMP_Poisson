...
// Método iterativo de relajación de Jacobi con OpenMP y colapso de bucles
void solve_poisson_jacobi_parallel(std::vector<std::vector<double>> &u, const std::vector<std::vector<double>> &rho, double h, double k, int max_iter, double tol) {
    int M = u.size() - 1;
    int N = u[0].size() - 1;
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        // Variable para reducción del max_error
...
void create_gnuplot_script(const std::string &datafile, const std::string &scriptfile) {
    ...
    gp << "pause -1 'Presione ENTER para cerrar'\n";
    gp.close();
}
...
int main() {
    ...
    solve_poisson_jacobi_parallel(u, rho, h, k, 10000, 1e-6);

    export_to_file(u, h, k, M, N, "solucion.dat");

    std::cout << "Generando script para gnuplot...\n";
